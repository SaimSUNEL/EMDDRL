...
import pickle
G = pickle.load(open("MIL/data/TwoRooms4XDistanceDict.pck", "rb"))

emdd_results = []
dd_results = []
eliminated_states = [422,423,424,425,426,
                     483,484,485,486,487,
                     544,545,546,547,548,
                     605,606,607,608]
average_count = 0
...
if DATASET == STEP_LIMIT_EQUALLY_BALANCED or DATASET == STEP_LIMIT_ONLY_POSITIVE:
    data_version = "ActionNoise(0.9)StepLimit(600)"
...
# Experiment folder
if algorithm == "EMDD":
    experiment_folder = "SpeedExperimentsTwoRooms4X/"+data_version+"/"+dataset_defs[DATASET]+"/"+algorithm+"/"+METHOD+"/"+SEARCH_SET+"/"+str(SKIPPING_FACTOR)
else:
    experiment_folder = "SpeedExperimentsTwoRooms4X/"+data_version+"/"+dataset_defs[DATASET]+"/"+algorithm

experiment_folder += "/#"+experiment_date+"#"+experiment_time

if not os.path.exists(experiment_folder):
    create_path = "SpeedExperimentsTwoRooms4X"
...
for experiment_id in range(1, 101):
    ...
    data_directory = "MIL/data/TwoRooms4XQAgentTrajectories/"
    experiment_number = experiment_id

    file_name = "TwoRooms4XBagExperiment["+str(experiment_number)+"].txt"
...
two_rooms_env_v2 = RLTwoRoomsEnvironment.RLTwoRoomsEnvironment('v9', is_step_restricted=True, step_limit=500)
