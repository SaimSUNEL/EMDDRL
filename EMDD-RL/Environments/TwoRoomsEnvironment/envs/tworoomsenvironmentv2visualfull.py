...
# state -> complete environment as an images

class TwoRoomsEnvironmentV2VisualFull(gym.Env):
    ...
    Note: a default reward range set to [-inf,+inf] already exists. Set it if you want a narrower range.
        ...
        self.row_tile_count = 21  # number of tiles in a row...
        ...
        self.states[4][10] = 0
        self.unused_states.remove(4*self.row_tile_count+10)

        self.states[5][10] = 0
        self.unused_states.remove(5 * self.row_tile_count + 10)
        ...
        self.image_bank = dict() # for each state an image...
        for y in range(self.column_tile_count):
            for x in range(self.row_tile_count):
                state = x+ y*self.row_tile_count
                self.image_bank[state] = self.getImage(state)
                # cv2.imwrite("img/"+str(state)+".png", self.image_bank[state])
    def getImage(self, state):
        tile_width = 5
        tile_height = 5

        image = np.zeros((tile_height * (self.column_tile_count),
                          (tile_width * (self.row_tile_count))), dtype=np.uint8)

        for y in range(self.column_tile_count):
            for x in range(self.row_tile_count):
                if self.states[y][x] == 0:  # empty
                    image[y * tile_height: (y + 1) * tile_height, x * tile_width: (x + 1) * tile_width] = 255
                elif self.states[y][x] == 1:
                    image[y * tile_height: (y + 1) * tile_height, x * tile_width: (x + 1) * tile_width] = 200
                elif self.states[y][x] == 2:
                    image[y * tile_height: (y + 1) * tile_height, x * tile_width: (x + 1) * tile_width] = 100
                    pass
        # current state is black...
        curr_y = state // self.row_tile_count
        curr_x = state % self.row_tile_count
        image[curr_y*tile_height: (curr_y+1)*tile_height, curr_x*tile_width: (curr_x+1)*tile_width] = 0
    ...
    def step(self, action):
        ...
        return self.image_bank[next_state], reward, done, {"issuccesful": finished_successfully}
    ...
    def reset(self):
        ...
        return self.image_bank[self.environment_current_state]
    ...
    def seed(self, seed=None):
        ...
        """
        return super(TwoRoomsEnvironmentV2, self).seed()
