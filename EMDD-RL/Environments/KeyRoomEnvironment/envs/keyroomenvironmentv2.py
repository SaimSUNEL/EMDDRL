...
class KeyRoomEnvironmentV2(gym.Env):
    ...
    Note: a default reward range set to [-inf,+inf] already exists. Set it if you want a narrower range.
        ...
        self.tile_width = 40  # px
        self.tile_height = 40  # px

        self.row_tile_count = 6  # number of tiles in a row...
        self.column_tile_count = 7  # number of tiles in a column

        self.screen_width = 500//2+24
        ...
        # RN (Simsek) example
        self.terminal_states = karguments["terminal_states"]
        self.unused_states = [] # walls...

        self.key_state_visited = False
        self.key_state = karguments["key state"]
        # actions
        self.UP = 3
        self.LEFT = 0
        self.RIGHT = 2
        ...
        self.unused_states.append(0 * self.row_tile_count + 5)
        self.states[0][5] = 2

        self.unused_states.append(1 * self.row_tile_count + 5)
        self.states[1][5] = 2

        self.unused_states.append(2 * self.row_tile_count + 5)
        self.states[2][5] = 2

        self.unused_states.append(2 * self.row_tile_count + 4)
        self.states[2][4] = 2

        self.unused_states.append(2 * self.row_tile_count + 3)
        self.states[2][3] = 2
        ...
        self.states[0][3] = 0
        self.unused_states.remove(0 * self.row_tile_count + 3)

        self.states[1][3] = 0
        self.unused_states.remove(1*self.row_tile_count+3)

        self.states[3][0] = 0
        self.unused_states.remove(3 * self.row_tile_count + 0)
        self.states[3][1] = 0
        self.unused_states.remove(3 * self.row_tile_count + 1)
        self.states[3][2] = 0
        self.unused_states.remove(3 * self.row_tile_count + 2)

        self.states[3][4] = 0
        self.unused_states.remove(3 * self.row_tile_count + 4)

        self.states[4][3] = 0
        self.unused_states.remove(4 * self.row_tile_count + 3)
        ...
        self.key_image = pygame.image.load("Key-free-to-use-cliparts.png")
        SCALE = 1.8
        self.resized_key_image = pygame.transform.scale(self.key_image, (int(self.key_image.get_width()/SCALE), int(self.key_image.get_height()/SCALE)))
    ...
    def step(self, action):
        ...
        if next_state == self.key_state:
            self.key_state_visited = True
        done = False
        is_succesfull = False
        if next_state in self.terminal_states:
            if reward > 0: # assumes successfull states emits positive reward...
                if self.key_state_visited is True:
                    print("***************HAS_FINISHED_SUCCESSFULLY**********")
        ...
        return next_state, reward, done, {"issuccesful": is_succesfull}
    ...
    def render(self, mode='human', close=False):
        ...
        if(self.is_window_created == False):
            ...
            self.font2 = pygame.font.SysFont("comicsansms", 10) # 8
        ...
        for y in range(self.column_tile_count):
            for x in range(self.row_tile_count):
                ...
                self.screen.blit(text2, ((MARGIN + self.tile_width) * x + MARGIN + 3,
                                         (MARGIN + self.tile_height) * y + MARGIN - 8 + 6))

                if state == self.key_state:
                    print("KEY IMAGE...")
                    self.screen.blit(self.resized_key_image, ((MARGIN + self.tile_width) * x + MARGIN +2,
                                         (MARGIN + self.tile_height) * y + MARGIN - 8 + 12))
    ...
    def seed(self, seed=None):
        ...
        """
        return super(KeyRoomEnvironmentV2, self).seed()
