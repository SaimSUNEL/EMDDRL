...
from Environments import RLKeyRoomEnvironment
...
start_time = timer()
import pickle
# G = pickle.load(open("../../MIL/data/SampleEnvironmentV0DistanceDict.pck", "rb"))

subgoal_results = []
eliminated_states = [31,32,37,38] # [91, 92, 102,80,81,90,101]
average_count = 0
...
# Experiment folder
if algorithm == "EMDD":
    experiment_folder = "SpeedExperimentsKeyRoomV1Environment(Online)/"+data_version+"/"+dataset_defs[DATASET]+"/"+algorithm+"/"+METHOD+"/"+SEARCH_SET+"/"+str(SKIPPING_FACTOR)
else:
    experiment_folder = "SpeedExperimentsKeyRoomV1Environment(Online)/"+data_version+"/"+dataset_defs[DATASET]+"/"+algorithm

experiment_folder += "/P("+str(POSITIVE_BAG_COUNT)+")/N"+str(NEGATIVE_BAG_COUNT)+")/#"+experiment_date+"#"+experiment_time

if not os.path.exists(experiment_folder):
    create_path = "SpeedExperimentsKeyRoomV1Environment(Online)"
...
for experiment_id in range(1, 101):
    ...
    # G = Graph.TransitionGraph.TransitionGraph(saved_file=G)  # for dynamic distance calculation
    transition_graph = Graph.TransitionGraph.TransitionGraph(None)

    included_experiment_count += 1

    data_directory = "../MIL/data/KeyRoomV1QAgentTrajectories/"
    experiment_number = experiment_id

    file_name = "KeyRoomV1BagExperiment["+str(experiment_number)+"].txt"
...
environment = RLKeyRoomEnvironment.RLKeyRoomEnvironment('v1', is_step_restricted=True, step_limit=500)
