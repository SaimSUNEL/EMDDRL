...
start_time = timer()
import pickle
# G = pickle.load(open("../MIL/data/TwoRooms5XEnvironmentDistanceDict.pck", "rb"))
...
# Experiment folder
if algorithm == "EMDD":
    experiment_folder = "AccuracyExperimentsTwoRoom5X(Online)/"+data_version+"/"+dataset_defs[DATASET]+"/"+algorithm+"/"+METHOD+"/"+SEARCH_SET+"/"+str(SKIPPING_FACTOR)
else:
    experiment_folder = "AccuracyExperimentsTwoRoom5X(Online)/"+data_version+"/"+dataset_defs[DATASET]+"/"+algorithm

experiment_folder += "/#"+experiment_date+"#"+experiment_time

if not os.path.exists(experiment_folder):
    create_path = "AccuracyExperimentsTwoRoom5X(Online)"
