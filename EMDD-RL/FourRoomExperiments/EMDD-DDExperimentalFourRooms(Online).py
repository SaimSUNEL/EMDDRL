...
FULL_BAG = lambda x: list(set(x)) # lambda x: x # lambda x: list(set(x))
...
# H_SET EXP 1->0.91, 2->0.91 3->0.95, 0.92
# H_SET LIN 1->0.77, 2->0.78, 3->0.80, 4->0.76
# H_ALL EXP 1->0.91, 2->0.91, 3->0.95, 4->0.92
# H_ALL LIN 1->0.82, 2->0.81, 3->0.84, 4->0.78
if(len(sys.argv)>2):
...
# Experiment folder
if algorithm == "EMDD":
    experiment_folder = "AccuracyExperimentsFourRooms(Online)/"+data_version+"/"+dataset_defs[DATASET]+"/"+algorithm+"/"+METHOD+"/"+SEARCH_SET+"/"+str(SKIPPING_FACTOR)
else:
    experiment_folder = "AccuracyExperimentsFourRooms(Online)/"+data_version+"/"+dataset_defs[DATASET]+"/"+algorithm

experiment_folder += "/#"+experiment_date+"#"+experiment_time

if not os.path.exists(experiment_folder):
    create_path = "AccuracyExperimentsFourRooms(Online)"
    ...
    for line in data_file:
        ...
        if DATASET == STEP_LIMIT_EQUALLY_BALANCED or DATASET == STEP_LIMIT_ONLY_POSITIVE:
            if parts[0] == "1":# len(parts) <= 201:
                ...
                positive_bags.append(FULL_BAG(bag))
                positive_episodes.append(bag)

            else:
                negative_bags.append(FULL_BAG(bag))
