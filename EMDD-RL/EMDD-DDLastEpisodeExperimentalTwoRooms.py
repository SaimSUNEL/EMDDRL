...
# Experiment folder
if algorithm == "EMDD":
    experiment_folder = "SpeedExperimentsLastEpisode/"+data_version+"/"+dataset_defs[DATASET]+"/"+algorithm+"/"+METHOD+"/"+SEARCH_SET+"/"+str(SKIPPING_FACTOR)
else:
    experiment_folder = "SpeedExperimentsLastEpisode/"+data_version+"/"+dataset_defs[DATASET]+"/"+algorithm

experiment_folder += "/#"+experiment_date+"#"+experiment_time

if not os.path.exists(experiment_folder):
    create_path = "SpeedExperimentsLastEpisode"
...
for experiment_id in range(1, 101):
    ...
    emdd = EMDD(positive_bags, negative_bags, G, STATES_TO_TEST, METHOD, distance_metric=DiverseDensity.DISTANCE_METRIC_GRAPH,
                H_CHECK=SEARCH_SET, SKIPPING_FACTOR=SKIPPING_FACTOR, SEED_SET=EMDD.SEED_SET_LAST_PATH_WITH_K)
    if algorithm == "EMDD":
        print("EMDD working")
...
two_rooms_env_v2.renderEnvironment()
if algorithm == "EMDD":
    two_rooms_env_v2.saveEnvironmentImage(experiment_folder+data_version+""+dataset_defs[DATASET]+""+algorithm+SEARCH_SET+METHOD)
else:
    two_rooms_env_v2.saveEnvironmentImage(experiment_folder+data_version+""+dataset_defs[DATASET]+""+algorithm)
