...
import pickle
G = pickle.load(open("MIL/data/TwoRoomsOnLeftDistanceDict.pck", "rb"))
...
# Experiment folder
if algorithm == "EMDD":
    experiment_folder = "SpeedExperimentsTwoRoomsOnLeft/"+data_version+"/"+dataset_defs[DATASET]+"/"+algorithm+"/"+METHOD+"/"+SEARCH_SET+"/"+str(SKIPPING_FACTOR)
else:
    experiment_folder = "SpeedExperimentsTwoRoomsOnLeft/"+data_version+"/"+dataset_defs[DATASET]+"/"+algorithm

experiment_folder += "/#"+experiment_date+"#"+experiment_time

if not os.path.exists(experiment_folder):
    create_path = "SpeedExperimentsTwoRoomsOnLeft"
...
for experiment_id in range(1, 31):
    ...
    data_directory = "MIL/data/TwoRoomsLeftQAgentTrajectories/"
    experiment_number = experiment_id

    file_name = "TwoRoomsOnLeftBagExperiment["+str(experiment_number)+"].txt"
...
two_rooms_env_v3 = RLTwoRoomsEnvironment.RLTwoRoomsEnvironment('v3', is_step_restricted=True, step_limit=500)
...
two_rooms_env_v3.setEnableSubgoalDisplay(True)

subgoals_dict = {}
for subgoal in elements:
    subgoals_dict[subgoal] = (counts.count(subgoal)+10)*5
print(subgoals_dict)
two_rooms_env_v3.setDisplaySubgoal(subgoals_dict)
two_rooms_env_v3.renderEnvironment()
if algorithm == "EMDD":
    two_rooms_env_v3.setEnvironmentImageTitle(data_version+""+dataset_defs[DATASET]+""+algorithm+SEARCH_SET+METHOD)
else:
    two_rooms_env_v3.setEnvironmentImageTitle(data_version+""+dataset_defs[DATASET]+""+algorithm)


two_rooms_env_v3.renderEnvironment()
if algorithm == "EMDD":
    two_rooms_env_v3.saveEnvironmentImage(experiment_folder+data_version+""+dataset_defs[DATASET]+""+algorithm+SEARCH_SET+METHOD)
else:
    two_rooms_env_v3.saveEnvironmentImage(experiment_folder+data_version+""+dataset_defs[DATASET]+""+algorithm)
