...
G = pickle.load(open("MIL/data/TwoRoomsOnRightDistanceDict.pck", "rb"))

emdd_results = []
dd_results = []
eliminated_states = [283,284,285,286,
                     324,325,326,327,
                     365,366,367,368,
                     406,407,408,409]
average_count = 0
...
if DATASET == STEP_LIMIT_EQUALLY_BALANCED or DATASET == STEP_LIMIT_ONLY_POSITIVE:
    data_version = "ActionNoise(0.9)StepLimit(1200)"
...
# Experiment folder
if algorithm == "EMDD":
    experiment_folder = "SpeedExperimentsTwoRoomsOnRight/"+data_version+"/"+dataset_defs[DATASET]+"/"+algorithm+"/"+METHOD+"/"+SEARCH_SET+"/"+str(SKIPPING_FACTOR)
else:
    experiment_folder = "SpeedExperimentsTwoRoomsOnRight/"+data_version+"/"+dataset_defs[DATASET]+"/"+algorithm

experiment_folder += "/#"+experiment_date+"#"+experiment_time

if not os.path.exists(experiment_folder):
    create_path = "SpeedExperimentsTwoRoomsOnRight"
...
for experiment_id in range(1, 31):
    # how many experiments are used for experiment

    included_experiment_count += 1

    data_directory = "MIL/data/TwoRoomsRightQAgentTrajectories/"
    experiment_number = experiment_id

    file_name = "TwoRoomsOnRightBagExperiment["+str(experiment_number)+"].txt"
...
two_rooms_env_v4 = RLTwoRoomsEnvironment.RLTwoRoomsEnvironment('v4', is_step_restricted=True, step_limit=500)
...
two_rooms_env_v4.setEnableSubgoalDisplay(True)

subgoals_dict = {}
for subgoal in elements:
    subgoals_dict[subgoal] = (counts.count(subgoal)+10)*5
print(subgoals_dict)
two_rooms_env_v4.setDisplaySubgoal(subgoals_dict)
two_rooms_env_v4.renderEnvironment()
if algorithm == "EMDD":
    two_rooms_env_v4.setEnvironmentImageTitle(data_version+""+dataset_defs[DATASET]+""+algorithm+SEARCH_SET+METHOD)
else:
    two_rooms_env_v4.setEnvironmentImageTitle(data_version+""+dataset_defs[DATASET]+""+algorithm)


two_rooms_env_v4.renderEnvironment()
if algorithm == "EMDD":
    two_rooms_env_v4.saveEnvironmentImage(experiment_folder+data_version+""+dataset_defs[DATASET]+""+algorithm+SEARCH_SET+METHOD)
else:
    two_rooms_env_v4.saveEnvironmentImage(experiment_folder+data_version+""+dataset_defs[DATASET]+""+algorithm)
