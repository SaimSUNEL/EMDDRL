...
import pickle
G = pickle.load(open("MIL/data/TwoRooms3XDistanceDict.pck", "rb"))

emdd_results = []
dd_results = []
eliminated_states = [352,353,354,355,356,
                     403,404,405,406,407,
                     454,455,456,457,458,
                     505,506,507,508]
average_count = 0
...
if DATASET == STEP_LIMIT_EQUALLY_BALANCED or DATASET == STEP_LIMIT_ONLY_POSITIVE:
    data_version = "ActionNoise(0.9)StepLimit(500)"
...
# Experiment folder
if algorithm == "EMDD":
    experiment_folder = "SpeedExperimentsTwoRooms3X/"+data_version+"/"+dataset_defs[DATASET]+"/"+algorithm+"/"+METHOD+"/"+SEARCH_SET+"/"+str(SKIPPING_FACTOR)
else:
    experiment_folder = "SpeedExperimentsTwoRooms3X/"+data_version+"/"+dataset_defs[DATASET]+"/"+algorithm

experiment_folder += "/#"+experiment_date+"#"+experiment_time

if not os.path.exists(experiment_folder):
    create_path = "SpeedExperimentsTwoRooms3X"
...
for experiment_id in range(1, 101):
    ...
    data_directory = "MIL/data/TwoRooms3XQAgentTrajectories/"
    experiment_number = experiment_id

    file_name = "TwoRooms3XBagExperiment["+str(experiment_number)+"].txt"
...
two_rooms_env_v2 = RLTwoRoomsEnvironment.RLTwoRoomsEnvironment('v8', is_step_restricted=True, step_limit=500)
