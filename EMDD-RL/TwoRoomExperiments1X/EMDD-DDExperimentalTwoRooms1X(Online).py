...
start_time = timer()
import pickle
# G = pickle.load(open("../MIL/data/TwoRooms1XEnvironmentDistanceDict.pck", "rb"))
...
# Experiment folder
if algorithm == "EMDD":
    experiment_folder = "AccuracyExperimentsTwoRoom1X(Online)/"+data_version+"/"+dataset_defs[DATASET]+"/"+algorithm+"/"+METHOD+"/"+SEARCH_SET+"/"+str(SKIPPING_FACTOR)
else:
    experiment_folder = "AccuracyExperimentsTwoRoom1X(Online)/"+data_version+"/"+dataset_defs[DATASET]+"/"+algorithm

experiment_folder += "/#"+experiment_date+"#"+experiment_time

if not os.path.exists(experiment_folder):
    create_path = "AccuracyExperimentsTwoRoom1X(Online)"
...
for experiment_id in range(1, 101):
    ...
    data_directory = "../MIL/data/TwoRooms1XQAgentTrajectories/"
    experiment_number = experiment_id

    file_name = "TwoRooms1XBagExperiment["+str(experiment_number)+"].txt"
...
environment = RLTwoRoomsEnvironment.RLTwoRoomsEnvironment("v6", is_step_restricted=True, step_limit=500)
